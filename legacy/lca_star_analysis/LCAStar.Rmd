---
title: "LCA Star Supplementary Code"
...
```{r fig.width=7.595238, fig.height=4.078740}
...
```{r fig.height=4.746032, fig.width=5.779527}
...
```{r include=FALSE}
# original histograms
g2b <- ggplot(subset(all_df.m, variable == "Walk"), aes(x=value, fill=Method))
g2b <- g2b + geom_histogram(binwidth=2, alpha=alpha_val) 
# g2b <- g2b + geom_density(adjust=6, alpha=0.8) 
g2b <- g2b + facet_grid(Method ~ Sample, scales = "free")
g2b <- g2b + theme(legend.position="none")
#g2b <- g2b + geom_vline(aes(xintercept=percent_25), colour=my_line_col, size=1.5, alpha=0.5, means)
#g2b <- g2b + geom_vline(aes(xintercept=median), colour=my_line_col, size=1.5, alpha=0.5, means)
#g2b <- g2b + geom_vline(aes(xintercept=percent_75), colour=my_line_col, size=1.5, alpha=0.5, means)
g2b <- g2b + xlab("Simple Distance")
g2b <- g2b + ylab("Frequency")
g2b
```
...
```{r fig.width=5.134921, fig.height=5.125984}
...
```{r fig.width=8.77, fig.height=6.897}
means <- select(all_df.m, Method, Sample, variable, value) %>%
...
```{r fig.width=5.968254, fig.height=5.456693}
