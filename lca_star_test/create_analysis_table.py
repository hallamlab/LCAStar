...
what_i_do = "Creates LCA_Star analysis table for downstream analysis."
...
def main(argv):
    ...
    read2origin = {} # hash to map input sequences to their original header
    ...
    ncbiID_to_taxaID = {} # ncbiID to taxaID
    taxaID_to_taxa = {} # ncbiID to taxa name
    for l in lines:
       fields = l.split("\t")
       fields = map(str.strip, fields)
    ...
    # now we should get the LCA results
    ...
    header = "\t".join(["contig","real","taxa","method","dist"])
    output.write(header + "\n") 
    for c in contig_to_orfs:
        ...
        line = "\t".join([contig, real, taxon, "LCA_Star", str(lcastar.get_distance(taxon, real))])
        output.write(line + "\n")
        taxon = lcastar.lca_majority(taxa_list) 
        line = "\t".join([contig, real, taxon, "Majority", str(lcastar.get_distance(taxon, real))])
        output.write(line + "\n")
        taxon = lcastar.getTaxonomy(lca_list) 
        line = "\t".join([contig, real, taxon, "LCA_Squared", str(lcastar.get_distance(taxon, real))])
        output.write(line + "\n")
